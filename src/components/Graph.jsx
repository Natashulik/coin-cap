...
const Graph = () => {
  ...
  const { historyData, interval, selectedButton } = useSelector(
    state => state.crypto
  );
  ...
  if (selectedButton === "1H") {
    updatedHistory = historyData.slice(-60).map((item, index) => {
      const min = new Date(item.date).toLocaleTimeString("en-US", {
        hour: "numeric",
        hour12: false,
        minute: "numeric",
      });
    ...
    updatedHistory = historyData.slice(-24 * 12).map((item, index) => {
      const hour = new Date(item.date).toLocaleTimeString("en-US", {
        hour: "numeric",
        hour12: false,
        minute: "numeric",
      });
  ...
  return (
    <div className="graph_block">
      <div className="switch_interval">
        <div
          className={
            selectedButton === "1H" ? "switch_button active" : "switch_button"
          }
          onClick={() => handleClickInterval("m1", "1H")}
        >
          {" "}
          1H{" "}
        </div>
        <div
          className={
            selectedButton === "24H" ? "switch_button active" : "switch_button"
          }
          onClick={() => handleClickInterval("m5", "24H")}
        >
          {" "}
          24H{" "}
        </div>
        <div
          className={
            selectedButton === "7D" ? "switch_button active" : "switch_button"
          }
          onClick={() => handleClickInterval("h1", "7D")}
        >
          {" "}
          7D{" "}
        </div>
        <div
          className={
            selectedButton === "1M" ? "switch_button active" : "switch_button"
          }
          onClick={() => handleClickInterval("d1", "1M")}
        >
          {" "}
          1M{" "}
        </div>
        <div
          className={
            selectedButton === "1Y" ? "switch_button active" : "switch_button"
          }
          onClick={() => handleClickInterval("d1", "1Y")}
        >
          {" "}
          Y{" "}
        </div>
