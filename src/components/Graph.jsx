...
const Graph= () => {
  ...
  const handleClickInterval = (interval, selectedButton)=> {
    ...
    if(selectedButton==='1H') {
        updatedHistory =  historyData.slice(-12).map((item, index)=>{
      ...
      updatedHistory =  historyData.slice(-24).map((item, index)=>{
        let hour = new Date(item.date).toLocaleString('en-US', { hour: 'numeric', hour12: true });
        let price = Number(item.priceUsd).toFixed(3);
           return {priceUsd: price, date: hour}
     }) 
    } else if(selectedButton==='7D') {
      updatedHistory =  historyData.slice(-7).map((item, index)=>{
      ...
      updatedHistory =  historyData.filter((item, index)=> index%30===0).map((item, index)=>{
